...
function buildUsagePriors(depth: Record<string, Record<string, Array<{ player_name: string; pos_rank: number }>>>): Record<string, any> {
  ...
  for (const [fantasyTeamId, posMap] of Object.entries(depth)) {
    priors[fantasyTeamId] = {};
    const byPos = posMap as Record<Position, Array<{ player_name: string; pos_rank: number }>>;

    // QB
    if (byPos.QB) {
      priors[fantasyTeamId].QB = byPos.QB.map((p, idx) => ({ player_name: p.player_name, snap_share: idx === 0 ? 0.95 : 0.05 }));
    }
    ...
    if (byPos.RB) {
      ...
      priors[fantasyTeamId].RB = byPos.RB.map((p, i) => ({ player_name: p.player_name, snap_share: Number((snapShares[i] || 0).toFixed(2)), rush_share: Number(((snapShares[i] || 0) * 0.9).toFixed(2)) }));
    }
    ...
    if (byPos.WR) {
      ...
      priors[fantasyTeamId].WR = byPos.WR.map((p, i) => ({ player_name: p.player_name, snap_share: Number((snapShares[i] || 0).toFixed(2)), target_share: Number((snapShares[i] || 0).toFixed(2)) }));
    }
    ...
    if (byPos.TE) {
      ...
      priors[fantasyTeamId].TE = byPos.TE.map((p, i) => ({ player_name: p.player_name, snap_share: Number((snapShares[i] || 0).toFixed(2)), target_share: Number((snapShares[i] || 0).toFixed(2)) }));
    }
  }
  return priors;
}
...
async function backfill(season: number) {
  ...
  const teamNameToId = teamsMap; // school name -> teamId
  const fantasyTeamIdToName = invert(teamsMap); // team_id -> school name
  ...
  while (true) {
    ...
    for (const doc of docs) {
      ...
      const fantasyTeamId = teamNameToId[teamName];
      if (!fantasyTeamId) continue;

      const keyTeam = fantasyTeamId;
  ...
  for (const [fantasyTeamId, posMap] of Object.entries(byTeam)) {
    for (const pos of Object.keys(posMap)) {
      ...
      byTeam[fantasyTeamId][pos as Position] = list.slice(0, 5);
    }
  }
