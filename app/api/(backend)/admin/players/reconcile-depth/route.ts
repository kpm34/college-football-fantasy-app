...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const [fantasyTeamId, posMap] of Object.entries(depthChart)) {
      for (const posKey of Object.keys(posMap as any)) {
        ...
        for (const entry of arr) {
          ...
          nameIndex.set(key, { teamId: fantasyTeamId, pos: posKey as Position });
        }
      }
    }
    ...
    while (true) {
      ...
      for (const doc of docs) {
        ...
        const expectedTeam = idToTeamName[match.fantasyTeamId] || doc.team;
