...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2) league_memberships: set client_id to auth_user_id when mismatched, and backfill display_name from Auth
    try {
      ...
      for (const m of memberships.documents as any[]) {
        ...
        try {
          if (currentClientId) {
            ...
            const display = u.name || u.email || 'Unknown User'
            const payload: any = {}
            if (m.client_id !== desiredId) payload.client_id = desiredId
            if (m.display_name !== display) payload.display_name = display
