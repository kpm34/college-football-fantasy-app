import { COLLECTIONS, DATABASE_ID, serverDatabases as serverDb } from '@/lib/appwrite-server'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ leagueId: string  }> }) {
  try {
    ...
    const league = await serverDb.getDocument(DATABASE_ID, COLLECTIONS.LEAGUES, leagueId)

    // Determine commissioner (new canonical + fallbacks)
    const commishId =
      (league as any).commissionerAuthUserId ||
      (league as any).commissioner ||
      (league as any).ownerClientId
    ...
    try {
      ...
      const rostersRes = await serverDb.listDocuments(
        DATABASE_ID,
      ...
      try {
        ...
        const allRosters = await serverDb.listDocuments(
          DATABASE_ID,
          COLLECTIONS.FANTASY_TEAMS,
          userQuery
        )
        ...
        team =
          allRosters.documents.find((r: any) => (r.leagueId || r.leagueId) === leagueId) || null
      } catch (fallbackError) {
