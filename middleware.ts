...
export function middleware(request: NextRequest) {
  ...
  const isDiagramApi = pathname.startsWith('/api/docs/diagrams')
  ...
  const enableCsp =
    process.env.NEXT_PUBLIC_ENABLE_CSP === 'true' || process.env.NODE_ENV !== 'development'
  if (enableCsp) {
    const csp = [
      "default-src 'self' https://accounts.google.com https://apis.google.com",
      // Allow Next.js live reload in preview, Appwrite SDK via jsDelivr, Google OAuth, and diagrams viewer scripts
      "script-src 'self' 'unsafe-inline' 'unsafe-eval' vercel.live https://accounts.google.com https://apis.google.com https://cdnjs.cloudflare.com https://cdn.jsdelivr.net https://*.gstatic.com https://viewer.diagrams.net https://app.diagrams.net",
      "style-src 'self' 'unsafe-inline' https://accounts.google.com https://fonts.googleapis.com",
      "img-src 'self' data: blob: https://lh3.googleusercontent.com https://*.gstatic.com https://*.googleusercontent.com",
      "connect-src 'self' https://nyc.cloud.appwrite.io wss://nyc.cloud.appwrite.io https://api.collegefootballdata.com https://*.vercel.app wss://*.vercel.app https://accounts.google.com https://apis.google.com https://oauth2.googleapis.com https://openidconnect.googleapis.com",
      "font-src 'self' data: https://fonts.gstatic.com https://fonts.googleapis.com",
      // Allow Google OAuth redirect flows and diagrams.net viewer in frames
      "frame-src 'self' https://accounts.google.com https://viewer.diagrams.net https://app.diagrams.net https://content.googleapis.com",
      isDiagramApi
        ? "frame-ancestors 'self' https://viewer.diagrams.net https://app.diagrams.net"
        : isDiagramAsset
...
export const config = {
  matcher: [
    '/((?!_next/static|_next/image|favicon.ico|manifest.json|service-worker.js|documentation/.*|docs/.*).*)',
  ],
}
