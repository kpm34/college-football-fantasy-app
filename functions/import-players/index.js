...
module.exports = async function (req, res) {
  ...
  // Use the function's built-in variables
  const endpoint = req.variables.APPWRITE_FUNCTION_ENDPOINT || process.env.APPWRITE_FUNCTION_ENDPOINT || 'https://nyc.cloud.appwrite.io/v1';
  const projectId = req.variables.APPWRITE_FUNCTION_PROJECT_ID || process.env.APPWRITE_FUNCTION_PROJECT_ID || 'college-football-fantasy-app';
  const apiKey = req.variables.APPWRITE_FUNCTION_API_KEY || process.env.APPWRITE_FUNCTION_API_KEY;

  if (!apiKey) {
    return res.json({
      success: false,
      error: 'API key not configured. Please set APPWRITE_FUNCTION_API_KEY in function variables.'
    });
  }
