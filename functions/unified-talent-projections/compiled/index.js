 ...
 */
async function loadEAData(season) {
    const eaMap = new Map();
    const filePath = node_path_1.default.join(process.cwd(), `data/ea/ratings_${season}.csv`);
 ...
 */
async function loadMockDraftData(season) {
    const draftMap = new Map();
    const filePath = node_path_1.default.join(process.cwd(), `data/mockdraft/${season}.csv`);
    if (!node_fs_1.default.existsSync(filePath)) {
 ...
 */
function calculateTalentMultiplier(talent, pos) {
    let multiplier = 1.0;
 ...
 */
async function analyzeSurroundingTalent(databases, dbId, teamId, pos, eaData) {
    try {
        // Get all players on the same team
        const teammates = await databases.listDocuments(dbId, 'college_players', [
            node_appwrite_1.Query.equal('team', teamId),
            node_appwrite_1.Query.limit(100)
        ]);
        let supporting_cast_ratings = [];
        let oline_ratings = [];
        for (const teammate of teammates.documents) {
            const tmPos = teammate.position;
            const tmName = teammate.name?.toLowerCase() || '';
 ...
 */
async function getESPNPlusAnalysis(playerName, teamId, pos) {
    try {
 ...
 */
async function buildTalentProfiles(databases, dbId, season) {
    ...
    const playersResponse = await databases.listDocuments(dbId, 'college_players', [
        node_appwrite_1.Query.greaterThan('fantasy_points', 0),
        node_appwrite_1.Query.equal('draftable', true),
        node_appwrite_1.Query.equal('position', ['QB', 'RB', 'WR', 'TE']),
        node_appwrite_1.Query.limit(50) // Test with 50 players first
    ]);
    ...
    try {
        const miRes = await databases.listDocuments(dbId, 'model_inputs', [
            node_appwrite_1.Query.equal('season', season),
            node_appwrite_1.Query.limit(1)
        ]);
        const model = miRes.documents.find((d) => d.week === undefined || d.week === null);
        if (model) {
            teamEff = parseMaybeJson(model.team_efficiency_json);
            pace = parseMaybeJson(model.pace_estimates_json);
        }
    }
    catch (error) {
    ...
    for (let i = 0; i < playersResponse.documents.length; i += batchSize) {
        ...
        for (const player of batch) {
            ...
            if (!playerName || !teamId || !posKey || !playerId)
                continue;
 ...
 */
function estimateDepthRank(fantasyPoints, position) {
    if (position === 'QB') {
        if (fantasyPoints > 350)
            return 1; // Starting QB
        if (fantasyPoints > 200)
            return 2; // Backup with some upside
        return 3; // Deep backup
    }
    if (position === 'RB') {
        if (fantasyPoints > 250)
            return 1; // RB1
...
async function getPreviousYearStats(databases, dbId, playerName, teamId, prevSeason) {
    try {
...
async function resolvePlayerId(databases, dbId, teamId, pos, playerName) {
    try {
        const players = await databases.listDocuments(dbId, 'college_players', [
            node_appwrite_1.Query.equal('name', playerName),
            node_appwrite_1.Query.equal('team', teamId),
            node_appwrite_1.Query.limit(1)
        ]);
        ...
        const playersNameOnly = await databases.listDocuments(dbId, 'college_players', [
            node_appwrite_1.Query.search('name', playerName),
 ...
 */
function computeEnhancedStatline(ctx) {
    const P = ctx.pace;
    const G = ctx.games;
 ...
 */
async function updatePlayerProjections(databases, dbId, ctx, season) {
    const stat = computeEnhancedStatline(ctx);
    const points = score(stat);
    try {
        // Update college_players table
        await databases.updateDocument(dbId, 'college_players', ctx.playerId, {
