...
export function DraftBoard({
  ...
  const renderCell = (roundIndex: number, slotIndex: number) => {
    ...
    return (
      <div
        ...
        className={`ui-tile-3d p-2 min-h-[56px] text-xs ${isCurrent ? 'ring-2 ring-red-600' : ''} ${pick ? 'pop-in' : ''}`}
        style={{ backgroundColor: pick ? bgColor : undefined, color: pick ? '#fff' : undefined }}
      >
        {pick ? (
          <div>
            <div className="truncate font-semibold leading-tight text-[12px]">
              {pick.playerName || '—'}
            </div>
  ...
  return (
    <div className="w-full h-full">
      <div className="px-4 py-3 ui-panel-3d">
      ...
      <div className="p-4 space-y-3">
        {Array.from({ length: rounds }).map((_, rIdx) => (
          <div key={rIdx} className="ui-panel-3d p-2">
