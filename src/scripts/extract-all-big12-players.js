...
const big12Teams = {
  ...
  'Kansas': { file: 'Kansas Jayhawks 2025 Rosters.md', abbreviation: 'KU' },
  'Kansas State': { file: 'Kansas State Wildcats 2025 Rosters.md', abbreviation: 'KSU' },
  'Oklahoma State': { file: 'Oklahoma State Cowboys 2025 Rosters.md', abbreviation: 'OKST' },
  'TCU': { file: 'TCU Horned Frogs 2025 Rosters.md', abbreviation: 'TCU' },
  'Texas Tech': { file: 'Texas Tech Red Raiders 2025 Rosters.md', abbreviation: 'TTU' }
};

async function addAllBig12TeamsFromPDFs() {
  console.log('🏈 Adding All Big 12 Teams from PDF Data...');
  console.log('==========================================');
  console.log('📋 Adding 13 Big 12 teams with current roster data');
  console.log('');
  ...
  for (const [teamName, teamInfo] of Object.entries(big12Teams)) {
    try {
      ...
      const filePath = path.join(process.cwd(), 'confrence rosters', 'Big_12_2025', 'markdown', teamInfo.file);
      ...
      for (const player of players) {
        try {
          ...
          console.log(`    ✅ ${player.name} (${player.position}) - ${player.rating || 3}★`);
          teamAdded++;
          totalAdded++;
        } catch (error) {
  ...
  console.log('\n🎉 All Big 12 Teams Summary:');
  console.log(`  ✅ Total players added: ${totalAdded}`);
  console.log(`  ❌ Total errors: ${totalErrors}`);
  console.log(`  📋 Teams processed: 13 (Arizona State, Arizona, BYU, Baylor, Cincinnati, Colorado, Houston, Iowa State, Kansas, Kansas State, Oklahoma State, TCU, Texas Tech)`);
  console.log('🏈 All Big 12 teams completed!');
}

addAllBig12TeamsFromPDFs(); 